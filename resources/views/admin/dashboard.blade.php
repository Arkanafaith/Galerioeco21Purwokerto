...
<div class="dashboard-header">
    ...
    <div style="display: flex; gap: 1rem;">
        ...
        <a href="{{ route('admin.content.index') }}" class="btn-primary-admin" style="background: #10b981;">
...
<div class="dashboard-grid">
    ...
    <div class="stat-card-admin" style="border-left: 4px solid #8b5cf6;">
        ...
        <div class="stat-content">
            ...
            <p class="stat-number">{{ number_format($todayVisitors) }}</p>
        </div>
    </div>
    ...
    <div class="stat-card-admin" style="border-left: 4px solid #10b981;">
        ...
        <div class="stat-content">
            ...
            <p class="stat-number">{{ number_format($totalVisitors) }}</p>
        </div>
    </div>
    ...
    <div class="stat-card-admin" style="border-left: 4px solid #f59e0b;">
        ...
        <div class="stat-content">
            ...
            <p style="font-size: 12px; color: #9ca3af; margin: 0;">{{ $totalReviews }} ulasan</p>
        </div>
    </div>
</div>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px;">
    ...
    <div style="background: white; border-radius: 12px; padding: 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.07); border: 1px solid #e5e7eb;">
        ...
        <div style="display: flex; flex-direction: column; gap: 12px;">
            <a href="{{ route('admin.products.create') }}" style="display: flex; align-items: center; gap: 12px; padding: 14px; background: #fef3c7; border-radius: 10px; text-decoration: none; color: #92400e; transition: all 0.2s;">
            ...
            <a href="{{ route('admin.products.index') }}" style="display: flex; align-items: center; gap: 12px; padding: 14px; background: #dbeafe; border-radius: 10px; text-decoration: none; color: #1e40af; transition: all 0.2s;">
                <span style="font-size: 20px;">📝</span>
                <div>
            ...
            <a href="{{ route('admin.content.index') }}" style="display: flex; align-items: center; gap: 12px; padding: 14px; background: #d1fae5; border-radius: 10px; text-decoration: none; color: #065f46; transition: all 0.2s;">
                <span style="font-size: 20px;">🎨</span>
                <div>
    ...
    <div style="background: white; border-radius: 12px; padding: 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.07); border: 1px solid #e5e7eb;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <a href="{{ route('admin.products.index') }}" style="font-size: 13px; color: #6b7280; text-decoration: none;">Lihat semua →</a>
        </div>
        <div style="display: flex; flex-direction: column; gap: 12px;">
            @forelse($recentProducts as $product)
            <a href="{{ route('admin.products.edit', $product->id) }}" style="display: flex; align-items: center; gap: 12px; padding: 10px; background: #f9fafb; border-radius: 8px; text-decoration: none; color: inherit; transition: all 0.2s;">
                ...
                <div style="flex: 1; min-width: 0;">
                    <p style="font-weight: 600; margin: 0; font-size: 14px; color: #1f2937; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $product->name }}</p>
                    <p style="margin: 0; font-size: 12px; color: #6b7280;">Rp{{ number_format($product->price,0,',','.') }}</p>
                </div>
                @if($product->is_best)
...
<div style="margin-top: 24px; background: linear-gradient(135deg, #1f2937 0%, #374151 100%); border-radius: 12px; padding: 24px; color: white;">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
        ...
        <div style="display: flex; gap: 24px; font-size: 13px; opacity: 0.9;">
            ...
            <span>📧 galerieco21purwokerto@gmail.com</span>
        </div>
    </div>
</div>
@endsection
